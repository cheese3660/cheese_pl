            ...
            if (json.is_object()) {
                if (json.contains(":template")) {
                    ...
                    for (auto &[key, value]: json.items()) {
                        ...
                        if (value.is_object()) {
                            if (value.contains(":insert-object")) {
                                auto ins = value[":insert-object"];
                ...
                for (auto &v: json) {
                    ...
                    if (v.is_object()) {
                        if (v.contains(":insert-array")) {
                            insertions.push_back({i, v[":insert-array"]});
                            sum += 1;
                        }
                    } else if (v == ":remove") {
                        insertions.push_back({i, nlohmann::json::array()});
                        sum += 1;
                    }
                    i += 1;
                }
                for (ptrdiff_t j = insertions.size() - 1; j >= 0; j--) {
                    ...
                    if (where < json.size())
                        json.erase(where);
                    for (ptrdiff_t k = insertion.second.size() - 1; k >= 0; k--) {
                        if (where == json.size()) {
                            json.push_back(insertion.second[k]);
                        } else {
                            json.insert(json.begin() + where, insertion.second[k]);
                        }
                    }
                }
                return sum;
